...
$statusClass = static function (string $state): string {
    return match ($state) {
        'active', 'available', 'detected' => 'success',
...
>
    <div class="row g-4">
        ...
        <div class="col-12 col-xl-5">
            <div class="card">
                <div class="card-header"><h2 class="h5 mb-0"><?php echo Text::_('COM_JEMPRESENTATION_INTEGRATION_STATUS'); ?></h2></div>
                <div class="card-body">
                    <dl class="jempresentation-status-list mb-0">
                        <div><dt><?php echo Text::_('COM_JEMPRESENTATION_RUNTIME_PLUGIN'); ?></dt><dd><span class="badge text-bg-<?php echo $statusClass((string) ($this->integrationStatus['runtime']['state'] ?? 'unknown')); ?>"><?php echo $this->escape((string) ($this->integrationStatus['runtime']['label'] ?? Text::_('COM_JEMPRESENTATION_STATUS_UNKNOWN'))); ?></span></dd></div>
                        <div><dt><?php echo Text::_('COM_JEMPRESENTATION_ACTIVE_TEMPLATE'); ?></dt><dd><?php echo $this->escape((string) ($this->integrationStatus['template']['label'] ?? Text::_('COM_JEMPRESENTATION_STATUS_UNKNOWN'))); ?></dd></div>
                        <div><dt><?php echo Text::_('COM_JEMPRESENTATION_HERO_BRIDGE'); ?></dt><dd><span class="badge text-bg-<?php echo $statusClass((string) ($this->integrationStatus['bridge']['state'] ?? 'unknown')); ?>"><?php echo $this->escape((string) ($this->integrationStatus['bridge']['label'] ?? Text::_('COM_JEMPRESENTATION_STATUS_UNKNOWN'))); ?></span><?php if (!empty($this->integrationStatus['bridge']['help'])) : ?><div class="small text-muted mt-1"><?php echo $this->escape((string) $this->integrationStatus['bridge']['help']); ?></div><?php endif; ?></dd></div>
                        <div><dt><?php echo Text::_('COM_JEMPRESENTATION_NATIVE_JEM_API'); ?></dt><dd><?php echo $this->escape((string) ($this->integrationStatus['native_api']['label'] ?? Text::_('COM_JEMPRESENTATION_STATUS_UNKNOWN'))); ?><?php if (!empty($this->integrationStatus['native_api']['help'])) : ?><div class="small text-muted mt-1"><?php echo $this->escape((string) $this->integrationStatus['native_api']['help']); ?></div><?php endif; ?></dd></div>
                    </dl>
                </div>
            </div>
            <div class="alert alert-info mt-3 mb-0"><?php echo Text::_('COM_JEMPRESENTATION_NON_ASSIGNED_UNCHANGED'); ?></div>
        </div>
    </div>

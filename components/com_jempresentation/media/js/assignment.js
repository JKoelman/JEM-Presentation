(() => {
    ...
    const init = () => {
        ...
        const update=()=>{
            const profile=registry.profiles?.[profileSelect.value]||{label:profileSelect.value||'-',status:'unknown',statusLabel:registry.labels?.status||'Unknown'};
            const layout=registry.layouts?.[layoutSelect.value]||{label:layoutSelect.value||'-',description:'',integrationLabel:'-',bridgeLabel:'-',status:'unknown',statusLabel:'-',preview:'unknown'};
            ...
            const warning=document.getElementById('jempresentation-runtime-warning'); if(warning){const unsupported=profile.status!=='available'||layout.status==='planned';warning.classList.toggle('d-none',!unsupported);warning.textContent=unsupported?(registry.labels?.unsupported||''):'';}
